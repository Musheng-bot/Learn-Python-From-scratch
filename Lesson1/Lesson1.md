...
python中的函数定义只需要使用一个关键字，叫作def，像下面这样就可以定义和调用一个函数了
...
2. 写一个函数，接受一个列表，返回它的平均值和方差，并把列表排序，提醒: $$ \bar{x} = \sum_{i=1}^n{x_i}$$ $$ var(x) = \frac{\sum_{i=1}^n{(x_i-\bar{x})^2}}{n}$$
...
9. 输入一个字符串，告诉我字符'a'在哪个索引处，如果不存在该字符也要返回一个特殊的值
